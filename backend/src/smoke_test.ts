import { PrismaClient, TechnicianStatus, ChecklistExecutionStatus, ChecklistItemStatus } from '@prisma/client';
import { execSync } from 'child_process';
import * as path from 'path';
import * as fs from 'fs';

// Force database URL to test_smoke.db
...
async function runSmokeTests() {
  ...
  // 2. ChecklistExecution create/read with status DRAFT, result null, version 1
  ...
  if (readExecution.status !== ChecklistExecutionStatus.DRAFT || readExecution.result !== null || readExecution.version !== 1) {
  ...
  // 3. ChecklistExecutionItem default NOT_CHECKED and unique index
  ...
  // 4. Cannot complete checklist if any item is NOT_CHECKED (Application constraint verification)
  ...
  const validateComplete = (items: any[]) => {
    const hasNotChecked = items.some(item => item.status === ChecklistItemStatus.NOT_CHECKED);
    if (hasNotChecked) {
  ...
  // 5. FAILED requires comment (Application constraint verification)
  console.log('Test 5: FAILED status requires comment');
  const validateItem = (status: ChecklistItemStatus, comment?: string | null) => {
    if (status === ChecklistItemStatus.FAILED && (!comment || comment.trim() === '')) {
      throw new Error('FAILED items must have an explanatory comment');
    }
  };

  try {
    validateItem(ChecklistItemStatus.FAILED, '');
  ...
  // 6. After COMPLETED, items or attachments cannot be modified (Application constraint verification)
  console.log('Test 6: Chặn chỉnh sửa sau khi COMPLETED');
  const checkCanModify = (status: ChecklistExecutionStatus) => {
    if (status === ChecklistExecutionStatus.COMPLETED) {
      throw new Error('Cannot modify items or attachments of a COMPLETED checklist execution');
    }
  };

  try {
    checkCanModify(ChecklistExecutionStatus.COMPLETED);
  ...
  // 7. WorkOrder has ChecklistExecution cannot be deleted due to Restrict relation
  ...
  // 8. Deleting User sets executedById and uploadedById to null (SetNull)
  ...
  // 9. Enum does not accept values outside the list
  console.log('Test 9: Enum type safety');
  // Try to create user with invalid status using ts-ignore or casting to check TS and DB
  try {
    await prisma.user.create({
      data: {
        ...
        role: 'TECHNICIAN',
        status: 'INVALID_STATUS' as any, // Cast to test Prisma validation
      },
    });
    throw new Error('Test 9 Failed: Invalid enum value accepted');
  } catch (e: any) {
    // Prisma will throw validation or database error
